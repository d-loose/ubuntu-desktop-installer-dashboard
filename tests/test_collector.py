...
class FakeSnapcraftResolver:
    ...
    def resolve_channel(self, name, channel, architecture):
        if not channel:
...
def test_collect_record_warns_when_subiquity_snap_channel_is_missing():
    responses = {
        PENDING_URL: '<a href="noble-desktop-amd64.iso">noble-desktop-amd64.iso</a> 2026-06-29 10:15\n<a href="noble-desktop-amd64.manifest">noble-desktop-amd64.manifest</a> 2026-06-29 10:16',
        MANIFEST_URL: "snap:ubuntu-desktop-bootstrap 26.04/stable/ubuntu-26.04.1 42\nsnap:snapd stable 24718\nsnapd 2.70+ubuntu1\n",
    }

    class MissingChannelSnapcraftResolver(FakeSnapcraftResolver):
        def resolve_revision(self, snap, architecture):
            resolved, warnings = super().resolve_revision(snap, architecture)
            if snap.name == "ubuntu-desktop-bootstrap":
                return PackageVersion(resolved.name, resolved.version, resolved.revision, None), warnings
            return resolved, warnings

    collector = Collector(lambda url: responses[url], FakeResolver(), MissingChannelSnapcraftResolver())

    record = collector.collect_record("noble", "amd64")

    assert record.subiquity_snap is None
    assert "Cannot resolve subiquity snap channel because channel is missing" in record.warnings
